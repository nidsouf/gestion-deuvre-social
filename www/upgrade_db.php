<?php
/**
 * upgrade_db.php - ترقية قاعدة البيانات بشكل آمن (إضافة أعمدة وفهارس ومشغلات)
 * قم بتشغيل هذا الملف مرة واحدة من المتصفح أو من سطر الأوامر.
 * http://localhost/upgrade_db.php
 */
require_once 'config/database.php';

// بدء معاملة (اختياري، لكن كل أمر يتم تنفيذه بمفرده)
try {
    $pdo->beginTransaction();
    
    // قائمة بالتعديلات المطلوبة: لكل تعديل نقوم بتنفيذ `try-catch` لتجنب أخطاء التكرار
    $alterStatements = [
        // جدول المستخدمين
    ...
    foreach ($alterStatements as $sql) {
        try {
            $pdo->exec($sql);
            echo "تم تنفيذ: " . substr($sql, 0, 50) . "...<br>\n";
        } catch (PDOException $e) {
            if (strpos($e->getMessage(), 'duplicate column name') !== false ||
                strpos($e->getMessage(), 'already exists') !== false) {
                echo "⚠️ تجاهل (العمود موجود مسبقاً): " . substr($sql, 0, 50) . "...<br>\n";
            } else {
                throw $e;
            }
        }
    ...
    // تحديث القيم الافتراضية للمستخدمين الحاليين (إذا كانت الأعمدة جديدة)
    ...
    echo "✓ تم تحديث القيم الافتراضية للمستخدمين.<br>\n";
    
    // إنشاء الفهارس (باستخدام IF NOT EXISTS)
    $indexStatements = [
    ...
    foreach ($indexStatements as $sql) {
        try {
            $pdo->exec($sql);
            echo "✓ فهرس: " . substr($sql, 0, 50) . "...<br>\n";
        } catch (PDOException $e) {
            echo "⚠️ خطأ في الفهرس (قد يكون موجوداً بالفعل): " . $e->getMessage() . "<br>\n";
        }
    }
    
    // إنشاء المشغلات (Triggers) – نفس ما ورد في ملف SQL
    $triggers = [
        // Employees
        ...
        END",
        // Budget triggers (check and update)
    ...
    // تنفيذ المشغلات (كل تشغيل يتم كأمرين: DROP ثم CREATE)
    for ($i = 0; $i < count($triggers); $i += 2) {
        ...
        try {
            $pdo->exec($dropSql);
            $pdo->exec($createSql);
            echo "✓ مشغل: " . substr($createSql, 0, 60) . "...<br>\n";
        } catch (PDOException $e) {
            echo "⚠️ خطأ في المشغل: " . $e->getMessage() . "<br>\n";
        }
    }
    ...
    $pdo->commit();
    echo "<hr><strong style='color:green;'>✅ تمت الترقية بنجاح! جميع الأعمدة والفهارس والمشغلات محدثة.</strong>";
    
} catch (PDOException $e) {
    $pdo->rollBack();
    echo "<strong style='color:red;'>❌ حدث خطأ: " . $e->getMessage() . "</strong>";
}
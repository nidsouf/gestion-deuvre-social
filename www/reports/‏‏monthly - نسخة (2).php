...
$is_print = isset($_GET['print']) && $_GET['print'] == 1; // وضع الطباعة مع التجميع
...
// استعلام أساسي (يجلب كل الاقتطاعات)
$sql = "
    SELECT 
        d.id,
        d.employee_id,
...
if ($is_print) {
    ...
    foreach ($deductions as $d) {
        ...
        $grouped[$key]['total_overall'] += $d['monthly_amount'] * $d['total_months'];
        $grouped[$key]['total_months'] += $d['total_months'];
        // تحديث التواريخ: أقدم بداية وأحدث نهاية
    ...
    // الوضع العادي: استخدام القائمة الأصلية
...
foreach ($dataSource as $item) {
    if ($item['category'] == 'Permanent') {
        $permanent[] = $item;
        $totalPermanent += ($is_print ? $item['total_monthly'] : $item['monthly_amount']);
    } else {
        $contract[] = $item;
        $totalContract += ($is_print ? $item['total_monthly'] : $item['monthly_amount']);
    }
}
...
<div class="report-container">
    ...
    <!-- نموذج الفلاتر (يظهر فقط في الوضع العادي، وليس في وضع الطباعة) -->
    <?php if (!$is_print): ?>
    <div class="filters">
        ...
        <tbody>
            ...
            <?php else: ?>
                ...
                <tr>
                    ...
                    <td><?= $is_print ? $item['total_months'] : $item['total_months'] ?> شهر</span></small></td>
                    <td><?= date('d/m/Y', strtotime($item['start_date'])) ?></span></small></td>
                    <td><?= date('d/m/Y', strtotime($item['end_date'])) ?></span></small></td>
                    <td>
                        <?= number_format($is_print ? $item['total_overall'] : $item['total_amount'], 2) ?> دج
                     </span></small></td>
                    <td>
                        <?php if($item['is_loan']): ?>
...
<?php
// إذا كان في وضع الطباعة، نضيف سكريبت لفتح نافذة الطباعة تلقائياً
if ($is_print): ?>
<script>
